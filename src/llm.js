...
const NUTRITION_JSON_SCHEMA = {
  ...
  properties: {
    ...
    water_oz: { type: 'number', description: 'fluid ounces of hydrating liquid. Plain water, sparkling water, and herbal tea COUNT (1 cup/glass ≈ 8 oz). Coffee, black tea, alcohol, juice, and milk do NOT count.' },
    omega3: { type: 'string', enum: ['Y', 'N'], description: 'Y if the meal contains a meaningful source (fatty fish, walnuts, flax, chia)' },
    confidence: { type: 'string', enum: ['low', 'medium', 'high'] },
  },
  required: ['calories', 'protein_g', 'calcium_mg', 'veg_servings', 'water_oz', 'omega3', 'confidence'],
...
Required JSON schema:
{
  ...
  "water_oz": number,           // fluid oz of hydrating liquid (plain water, sparkling water, herbal tea COUNT at ~8 oz/cup; coffee, black tea, alcohol, juice, milk do NOT)
  "omega3": "Y" | "N",          // Y if the meal contains a meaningful source (fatty fish, walnuts, flax, chia)
  "confidence": "low" | "medium" | "high"
}

Be conservative. Round calories/calcium to nearest 10, protein to nearest 1, veg_servings to 0.5, water_oz to nearest 2. Use values from typical USDA food data. If the input is empty or non-food, return all zeros with confidence "low".`
...
export async function estimateNutrition(foodDescription, { recipes = [], signal } = {}) {
  ...
  return {
    ...
    water_oz: Math.round((Number(parsed.water_oz) || 0) / 2) * 2,
